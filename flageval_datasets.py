...
class BUSTMDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        prompt += f"\nB. 不相似"
        prompt += f"\n答案: \n"
        sample = {"prompt": format_prompt(prompt), "answer": self.prompt_dict[answer]}
        return sample
...
class RAFTDataset(Dataset):
    ...
    def __generate_prompt__(self, ban_index=-1):
        ...
        # 10.17 update: 从官方获取了 Instruction 可以直接字典提取 instruction 了。
        # prompt=self.first_line_dict[sub_name]+'\n\n'
...
class EPRSTMTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {"prompt": format_prompt(prompt), "answer": self.prompt_dict[question["label"]]}
        return sample
...
class TNEWSDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        prompt += f"\nO:电竞"
        prompt += f"\n答案:\n"
        sample = {"prompt": format_prompt(prompt), "answer": self.prompt_dict[label]}
        return sample
