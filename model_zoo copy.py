...
class LinlyLLaMA2(BaseLLM):  # Have BUGS
    def __init__(self, model_name, model_path, tokenizer_path, config_path="") -> None:
        self.name = model_name
        self.tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_path, trust_remote_code=True
...
class Llama(BaseLLM):
    def __init__(self, model_name, model_path, tokenizer_path, config_path="") -> None:
        ...
        self.model = (
            ...
            AutoModelForCausalLM.from_pretrained(model_path) #.half().cuda() #, trust_remote_code=True)
            .bfloat16()
            .cuda()
        )
    ...
    def inference(self, queries, choiceses, use_logits, nt):
        ...
        if use_logits:
            ...
            # outputs = self.model(**inputs)# return_last_logit=True)
            logits = outputs.logits[:, -1] # 提取最后一个logit，作为分类的依据
...
class Llama2(BaseLLM):
    def __init__(self, model_name, model_path, tokenizer_path, config_path="") -> None:
        ...
        self.model = (
            AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True,device_map="auto") #.half().cuda() #
            .bfloat16()
            .cuda()
        )
