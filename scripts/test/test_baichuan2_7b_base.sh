MODEL=BaiChuan2_base
LLAMA_BASE=/mnt/store/Baichuan2-7B-Base/
current_datetime=$(date +"%m%d_%H_%M_%S")
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5
# torchrun --nproc-per-node 1 main_v2dist.py --dataset-names='BoolQ' \
mkdir -p "./evaluation_results/$(basename ${LLAMA_BASE})"
torchrun --nproc-per-node 6 main_v2dist.py --dataset-names='BoolQ' \
# python main_v2.py --dataset-names='BoolQ' \
    --model-name $MODEL \
