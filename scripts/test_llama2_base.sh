...
# DATASET_LIST=("BoolQ" "MMLU" "TruthfulQA" "RAFT" "Chinese_MMLU" "C-Eval" "GAOKAO2023")
DATASET_LIST=("IMDB"  "CSL" "ChID" "CLUEWSC" "EPRSTMT" "TNEWS" "OCNLI" "BUSTM") 

for dataset in "${DATASET_LIST[@]}"; do
    current_datetime=$(date +"%m%d_%H_%M_%S")
    {
        CUDA_VISIBLE_DEVICES=7 python main_v2.py --dataset-names="$dataset" \
        --model-name $MODEL \
        --model-path $LLAMA_BASE \
        --tokenizer-path $LLAMA_BASE \
        --batch-size 8 \
        --verbose 
        # --no-save
    } > "./logs/new_dataset_log/${dataset}_${current_datetime}_llaba2.log" 2>> "./logs/new_dataset_log/${dataset}_${current_datetime}_llama2_error.log"
done